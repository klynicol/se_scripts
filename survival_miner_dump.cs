...
const float MIN_SAFE_DISTANCE = 2f; // Minimum safe distance from ground
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    string depthReadout = "Distance to ground: N/A";
    if (depthCamera.CanScan(MAX_SCAN_DISTANCE))
    {
        ...
        {
            Echo("here1");
            double distanceToGround = Vector3D.Distance(depthCamera.GetPosition(), hit.HitPosition.Value);
            depthReadout = $"Distance to ground: {distanceToGround:F2}m";

            // Optional: Add warning if too close to ground
            if (distanceToGround < MIN_SAFE_DISTANCE)
            {
                Echo("Here2");
                depthReadout += " WARNING: Too close to ground!";
            }
        }
    }
    else
    ...
    if (mergeDrillRetract.Enabled && landerState == LanderState.Retracted)
    {
